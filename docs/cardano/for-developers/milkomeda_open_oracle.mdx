...
The Milkomeda Open Oracle (MOO) is a decentralized oracle service for a USD/ADA pricefeed with a minimal governance system. It enables multiple owners to post data, and the majority of the current owners can add or remove owners. 
...
Those interested in going through the Smart Contract, can check out the in the [public repo](https://github.com/unitedx-finance/A3-Oracle).
...
|---|---|
| Testnet | 0x2a16a70E71D2C6f07F02b221B441a2e35E3d0848 |


It is crucial to note that to consume the feed of the Milkomeda Open Oracle, the caller of the read function must agree to the terms of service, which are accessible [here](https://ext-oracle-disclaimer.milkomeda.com/disclaimer.pdf).  This is a one-time requirement (per address), and we will provide guidance on how to accomplish that in this guide.
...
**
Accepted Terms of Service: true<br></br>
Price: 0.352135000000000024
**
...
**
Connected to Milkomeda C1 Testnet: True<br></br>
Accepting ToS...<br></br>
Price: 0.352135
**
...
The Milkomeda Open Oracle provides a price feed for the price of ADA in USD, while some times the inverse of this price is desired, i.e. the price of USD in ADA.

With this in mind, another smart contract with the same interface is provided, and that outputs the price feed of USD in ADA.

To consume this inverted price feed, the caller address needs to acept the terms and conditions of the Milkomeda Open Oracle, to be consumed by this alternative smart contract, althought it's a one time process.


All the steps detailed above for the two ways of consuming the price feed, either inside a smart contract or calling the oracle with Javascript or Python, would be exactly the same since it has a common interface, and the only part that would differ is the address.

The following table contains the deployment addresses for the smart contract for the feed of USD in ADA.
...
Calling the inverted price feed with javascript:
...
would show an output similar to this:
