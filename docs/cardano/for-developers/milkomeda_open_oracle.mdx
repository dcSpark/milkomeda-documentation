...
As explained above, the Milkomeda Open Oracle provides a price feed for the price of USD in ADA, while may time the inverse of this price is desired, i.e. the price of ADA in USD.

With this in mind, another wrapper smart contract with the same interface is provided, and that outputs the price feed of ADA in USD.

To consume this inverted price feed, one needs to acept the terms and conditions of the origin Milkomeda Open Oracle, to be consumed by the wrapper, althought it's a one time process.


All the steps detailed above for the two ways of consuming the price feed, either inside a smart contract or calling the oracle with Javascript or Python, would be exactly the same for the wrapper smart contract since it has a common interface, and the only part that would differ is the address.

The following table contains the deployment addresses for the wrapper smart contract.
