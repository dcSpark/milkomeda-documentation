...
:::
The transfer will require 10 confirmations, which should take approximately **5** minutes. Once the transaction is confirmed, the transferred mTADA should be visible in your Milkomeda Cardano Devnet wallet in MetaMask.
...
Now that we have successfully wrapped Test ADA into mTADA for use on the Milkomeda Cardano Devnet sidechain, the next step is to deploy a sample smart contract.

:::caution mTADA and Wrapped Test ADA

As we have seen, mTADA is the base asset used for paying fees and gas on the Milkomeda Cardano Devnet sidechain. When you move Test ADA to Milkomeda, you receive mTADA.

"Wrapped Test ADA" (wTADA) is a separate ERC20 token that functions the same as WETH on Ethereum.

The contract address for wTADA can be found below. Note that although you can send mTADA to this address to wrap it, you **shouldn't** send wTADA to this address to unwrap it but rather interact with it using the right method in the smart-contract. wTADA contract address: 0x65a51E52eCD17B641f8F0D1d56a6c9738951FDC9
