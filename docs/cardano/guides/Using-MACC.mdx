...
:::info
Suppose we establish a Asset Consolidation Contracts for mUSD (Canonical asset). User A transfers 10ceUSDC to this contract and receives 10mUSD in exchange. Next, user B sends 20multiUSDC to the smart contract and receives 20mUSD in return. Previously, users A and B would have been required to deposit their funds in separate liquidity pools, such as ceUSDC/mADA and multiUSDC/mADA. However, by canonizing their assets into mUSD, they can now combine their liquidity in the same pool, mUSD/mADA.
:::
...
**Following infographics aims to show how MACC and Treasury works:** (TBD)
...
:::note
On the mainnet deployment, you should see mUSD, which represents the Canonical token of USDC-fragmented assets.
::::
...
>3.2 Step 2: Confirm the conversion transaction in your Metamask wallet. 
...
After the network has processed your transaction, you have successfully converted your Canonical asset back to a Fragmented token. If this is the first time you have held this Fragmented token in your wallet, you can add it to your MetaMask wallet by clicking the **Add token to wallet** button.
