...
:::tip
If that is not the case, please refer to the [documentation on configuring Metamask](../for-developers/configuring-metamask-for-devs) and [obtaining test ADA on Milkomeda C1 Testnet](../for-developers/obtaining-test-ada).
:::
 ...
 **Next, we’ll select the network to deploy to**. However, Milkomeda C1 Testnet will not be listed by default, we can click on Configure Networks to add it. 
 
 There we can simply type Milkomeda in the search input and the 4 chains will be available to choose from: C1 and A1, mainnet and testnet. 
...
- First to deploy the contract on Milkomeda C1 Testnet, 
...
To integrate Milkomeda C1 Testnet into our project, we need to make a minor change. We'll import the chain from "@thirdweb-dev/chains" and add it to the ThirdWebProvider by editing the "src/main.tsx" file.
