...
If the result if `true`, you will have to wait a bit more for the callback to be made and check again after a few seconds. If the result is `false`, the callback was made to the smart contract and you can check the latest request again by running that task again:
