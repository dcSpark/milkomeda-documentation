...
The implementation on mainnet would be the same, and the only change would be the request parameters that are stored in the smart contract and that point to the provider in the smart contract.
...
:::info Designated Sponsor Wallets
...
The first tasks makes the request and the second one checks the result of the latest request. The output should look something like this:
...
If the result if true, you will have to wait a bit more for the callback to be made and check again after a few seconds. If the result is `true`, the callback was made to the smart contract and you can check the latest request again by running that task again:
