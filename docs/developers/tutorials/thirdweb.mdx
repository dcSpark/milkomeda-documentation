...
Thirdweb provides a CLI to create development environment based on a few templates. So to set up our environment, run the following:
...
1. Framework to use? Hardhat or Forge. We’ll use Hardhat
1. Name of Smart Contract? Let’s call it SimpleVoting
1. Type of contract? Empty Contract

Smart contracts in Solidity won’t be the focus of this article so we will use a (...)
...
If that is not the case, please refer to the milkomeda docs at the following link to configure Metamask and obtain test ADA.
