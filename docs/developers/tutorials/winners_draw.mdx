# Getting Winners for a draw “randomly” using a Smart Contract

Milkomeda ran a survey last week asking several questions to understand users’ perceptions of its products and services. The results were quite positive and a summary can be found in the following Twitter post link

One bonus for answering the survey was entering into a draw for the chance to be one of the lucky winners of a prize of 400 ADA each. 

We thought the draw provides an interesting use case for a simple smart contract that we can use to not only determine the winners but also to store the result where everyone can see and verify. We will build this example in solidity and use it to show how to deploy a smart contract on a Milkomeda chain.

In summary, the smart contract will be deployed with a boolean to verify if the draw has taken place or not, a fixed size array of 10 (winners), and a function that can only be called by the contract deployer to run the draw by generating 10 random numbers between 1 and the total numbers of answers of the survey, and storing them in the array. The 10 random numbers would define the winners from a pre-published ordered list of entries.

We will then briefly show how to deploy the contract using Hardhat, Foundry and Apeworx.
...
## Deploy and Test Contract on C1 and A1 Testnets

Now that we have written our smart contract, let’s see how we could use four different development tools, Hardhat, Foundry, Brownie and Apeworx, to deploy it on the Milkomeda testnets.

Installing and using these tools will not be the focus here, and what we want to highlight is how to set up these frameworks to work with the milkomeda chains.

Also, the steps for deploying on C1 or A1, mainnet or testnet, are exactly the same, considering one is using a funded account, so we will use C1 Testnet for this example. Please refer to the milkomeda docs here for a walkthrough on the details of each chain, and how to get funds on testnet.
