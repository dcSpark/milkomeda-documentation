...
At the time of this research and the implementation of the DA solution on top of Algorand Box Storage wasn't available. Currently, the team is working on a solution that will allow us to store large amounts of data on Algorand using Box Storage which also can work with the implementation of fraud proofs.

## A Naive Transaction Note Approach

Algorand has transaction notes (similar to metadata in other systems) which allow up to 1 kb of data to be included in a given transaction. Furthermore, Algorand allows up to 16 transactions to be grouped together such that all 16 txs must all be included in a block at the same time for any one of them to be considered valid.
...
The batches can be read from the transaction note data from the block, by deconstructing the grouped transactions into their notes, rebuilding the batch from it’s 16 parts, and then submitting said batch data to the rollup node to update the current state of the rollup from the node’s perspective. Rather straightforward all in all.
...
Theoretically multiple batchers can post batches in a single block, however with the first version of our rollups the invalid batches are simply ignored. This means that if a bad actor submits the same tx to two different batchers and they both include the tx, only the first batch will be valid. Thus unless we create a protocol on top to address this, we must assume that only a single batch per block will be posted.
...
The rollup node now takes the batch which has been verified to be available, and validates if the transactions in the batch are all valid (this happens 100% off chain, not on Algorand).
...
Depending on the complexity of the implementation, if specific batchers repeatedly keep failing to submit the data of batches they submit, it would be wise to stake slash them for attempting to DoS the system or simply failing to ensure their software is working properly.
...
The naive transaction note approach is extremely limited due to the 16kb limit. A max TPS of 6.33 is insufficient for our needs and would likely cause us to take a reputational hit even if it was a first step due to how limited the throughput would be.
...
For Milkomeda we will likely start out with the basic merkelized batch proposal approach, and move towards the advanced approach over time. The basic approach provides us with the majority of the benefits, however is simpler and as such is more centralized as a tradeoff.
