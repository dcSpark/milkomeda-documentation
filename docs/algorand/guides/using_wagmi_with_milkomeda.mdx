...
This article will guide you through the process of **developing a simple Storage smart contract**, deploying it on Milkomeda A1 Devnet, and accessing it using a frontend built with React, Wagmi, and Rainbowkit.
...
5. Edit **hardhat.config.js** to add the A1 Devnet and your funded test account. Refer to the [Milkomeda docs](../for-developers/obtaining-test-algo) for information on how to get devnet funds.
...
7. Run the deployment script on A1 Devnet:
...
By default, only Ethereum mainnet will be available so let’s change that to add Milkomeda A1 devnet. Create a file named **milkomeda.ts** in the folder, with the following:
...
By clicking on the chain drop-down, we will now be able to switch network to Milkomeda A1 Devnet.
...
**With that, we now have a fully functional dApp that connects to the Milkomeda A1 Devnet, can read from our Smart Contract, and is also capable of writing to it.**
<img
  src={img4}
  className="img-full"
  alt="Full dApp"
...
This guide provided a step-by-step explanation of how to develop a basic Storage Smart Contract and deploy it on the Milkomeda A1 Devnet using Hardhat. It also outlined how to create a frontend that interfaces with the contract, utilizing React, Wagmi, and Rainbowkit.
