...
This guide will show you how to deploy a simple smart contract to Milkomeda A1, an EVM-based rollup connected to Cardano.
...
First, connect your wallet using one of the available options. We will use Metamask and we’ll assume that Milkomeda C1 Testnet is already configured and there is an account with test funds (mTADA). 

If that is not the case, please refer to the [doc on configuring Metamask](../for-developers/configuring-metamask.mdx) and [obtaining test ALGO](../for-developers/obtaining-test-algo.mdx).
...
/>
 ...
 Next, we’ll select the network to deploy to. However, Milkomeda A1 Testnet will not be listed by default, but we can click on Configure Networks to add it. There we can simply type Milkomeda in the search input and the 4 chains will be available to choose from: A1 and C1, mainnet and testnet. 
...
/>
 ...
 After selecting Milkomeda A1 Testnet, the chain parameters will already be filled in and we only need to click “Add Network”.
...
Now we can click on “Deploy Now” and two transactions will be sent, one to deploy the contract on Milkomeda A1 Testnet and a second one to add the contract to the Thirdweb dashboard. The second transaction will actually only be signed by metamask and is a gassless transaction.
...
One first change that is needed is to add the Milkomeda A1 Testnet to the list of available chains. To do that, we’ll edit the “src/main.tsx” and import the chain from “@thirdweb-dev/chains” and add it to the ThirdWebProvider.
...
Great, users can now vote! But how can se see the results? Let's add a component for that. 
...
Bonus: Let's replace the simple numbers showing the result with a graph, using graphjs
...
We have create and deployed a smart contract on Milkomeda A1 Testnet with a lot of the boilerplate taken care of by Thirdweb. Building the DApp to interact with our smart contract was also very simple. With the provided React hooks, we did not need to provide ABI and reading and calling the smart contract was done with very few lines of code.
