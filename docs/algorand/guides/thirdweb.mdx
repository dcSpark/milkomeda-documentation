...
**First, connect your wallet using one of the available options.** We will use Metamask and we’ll assume that Milkomeda A1 Testnet is already configured and there is an account with test funds (mTALGO). 
:::tip
...
**Next, we’ll select the network to deploy to**. However, Milkomeda A1 Testnet will not be listed by default, we can click on Configure Networks to add it. 
 
 There we can simply type Milkomeda in the search input and the 4 chains will be available to choose from: A1 and C1, mainnet and testnet. 
 <img
  src={img4}
  className="img-full"
...
- First to deploy the contract on Milkomeda A1 Testnet, 
...
To integrate Milkomeda A1 Testnet into our project, we need to make a minor change. We'll import the chain from "@thirdweb-dev/chains" and add it to the ThirdWebProvider by editing the "src/main.tsx" file.
