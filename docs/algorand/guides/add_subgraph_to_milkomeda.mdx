...
In this guide we will learn how to deploy a subgraph for smart contracts on Milkomeda A1, using TheGraph.

Milkomeda chains are still not supported on The Graph's decentralized network, but one can run his own Graph Node to index any EVM-compatible network.

We will first go through the steps of creating a subgraph in a local environment so that we can test and develop our smart contract and, once we are happy with the result, we will deploy it on a Milkomeda A1 Testnet.
