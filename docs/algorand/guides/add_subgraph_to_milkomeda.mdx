...
To start, we will guide you through the steps of creating a subgraph in a local environment so you can test and develop your smart contract. Once you are satisfied with the result, we will show you how to deploy it on a Milkomeda A1 Devnet.
...
To be able to deploy to the local ganahache chain in docker container, we'll have to add it to the `hardhat.config.ts`. We will also already add the Milkomeda A1 Devnet.
...
## Deploying a Subgraph for Milkomeda A1 Devnet

After testing that our contract and subgraph are working well together, we can deploy to a devnet.
The changes required to deploy to Milkomeda A1 Devnet are very simple.

From the current setup, we only need to change two things:
1. The `ethereum` environment variable in the docker-compose should now point to the Milkomeda A1 Devnet RPC:
...
We can now query our subgraph and it will have indexed the events from Milkomeda A1 Devnet.
