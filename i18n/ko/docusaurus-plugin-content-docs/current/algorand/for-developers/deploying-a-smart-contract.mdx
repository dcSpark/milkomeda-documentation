...
그러면 새 Solidity 스마트 계약의 파일 이름을 입력하라는 메시지가 표시됩니다. 여기서는 ERC-20 스마트 계약을 배포하기 위해 "Test_ERC20.sol"이라는 파일 이름을 붙이기로 합시다. 파일 이름을 입력하고 **Enter**를 누릅니다. 커서가 편집 패널로 이동하므로 Solidity 코드를 입력합니다. 코드는 다음과 같습니다.
